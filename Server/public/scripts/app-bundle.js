  ...
  function () {
    ...
    _proto.changeFiles = function changeFiles() {
      ...
      var _loop = function _loop(i) {
        var addFile = true; // eslint-disable-next-line no-loop-func

        _this.filesToUpload.forEach(function (item) {
          if (item.name === _this.files[i].name) addFile = false;
        });

        if (addFile) _this.filesToUpload.push(_this.files[i]);
      };
...
define('modules/home',["exports"], function (_exports) {
  "use strict";
      ...
      this.httpClient.configure(function (config) {
        config.withBaseUrl(_this.BASE_URL).withDefaults({
          ...
          request: function request(_request) {
            // eslint-disable-next-line no-console
            console.log('Requesting ${request.method} ${request.url}'); // eslint-disable-next-line vars-on-top
      ...
      regeneratorRuntime.mark(function _callee2(id) {
        ...
        return regeneratorRuntime.wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                url = this.HELP_TICKET_CONTENT_SERVICE + '/helpTicket/' + id; // eslint-disable-next-line eqeqeq
